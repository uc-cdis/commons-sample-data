...
for c in res["Contents"]:
    ...
    if len(paths) < 3:
        ...
        if len(found_files) == 1:
            ...
            try:
                mj_id = wrp.get_metadata_json_file_metadata(bundle_id)["content"][0][
                    "id"
                ]
            except KeyError:
                ...
                try:
                    s3_client.download_fileobj(
                        REDWOOD_BUCKET, "{}/{}".format("data", mj_id), metadata_json
                    )
                ...
                res = s3_client.upload_fileobj(
                    metadata_json, COMMONS_BUCKET, "{}/metadata.json".format(new_folder)
                )

                s3_res = boto3.resource("s3")
                copy_source = {"Bucket": COMMONS_BUCKET, "Key": c["Key"]}
                filesize = s3_client.head_object(**copy_source)["ContentLength"]
                with tqdm(
                    total=filesize, unit="B", unit_scale=True, desc=c["Key"]
                ) as bar:
